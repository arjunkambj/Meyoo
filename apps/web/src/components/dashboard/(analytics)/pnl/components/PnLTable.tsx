...
> = {
  ...
  netProfit: {
    ...
    category: "profit",
    isBold: true,
    rowTintClass: "bg-surface-secondary",
  },
  netProfitMargin: {
    label: "Net Margin %",
...
export const PnLTable = React.memo(function PnLTable({
  ...
  const tableContent = useMemo(() => {
    ...
    const renderMetricRow = (
      ...
      const rowTintClass =
        config.rowTintClass ??
        (isBoldRow ? "bg-surface-secondary" : "");
            ...
            <div className="flex items-center gap-2">
              {config.icon && (
                <Icon
                  ...
                  className={config.iconColor || "text-foreground"}
                />
              )}
              <span
                className={`
                ${isBoldRow ? "text-sm font-semibold" : isSubItem ? "text-xs text-foreground" : "text-sm text-foreground"}
              `}
              >
                {config.label}
              </span>
